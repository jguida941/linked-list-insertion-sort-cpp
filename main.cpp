...
// Enable by compiling with -DTRACE (or CMake -DTRACE=ON).
static void DebugPrint(const char* msg, const List* list) {
#ifndef TRACE
    // Avoid "unused parameter" warnings when tracing is disabled.
    (void)msg; (void)list;
#else
    std::cout << msg << "  ";
...
// --- ListPrepend ---
// Puts a new node at the very front of the list, making it the new head.
...
// --- ListInsertAfter ---
// Puts `newNode` into the list immediately after the `prev` node.
...
// --- ListRemoveAfter ---
// Removes the node that comes right after `prev`.
...
// --- FindInsertionSpot ---
// Finds the node that should come right BEFORE a new value in the sorted part of the list.
// Returns `nullptr` if the value should be the new head.
...
/**
 * @brief Sorts the list using the insertion sort method.
 */
    ...
    while (curr != nullptr) {
...
#ifdef TRACE
        traceui::print_state("State: BEFORE place (H=head, P=prev, C=curr, N=next, S=spot)",
                             list->head,
        ...
        // --- CASE 2: `curr` needs to be moved. ---
...
#ifdef TRACE
            traceui::print_state("State: AFTER unlink (curr isolated)",
                                 list->head,
                                 traceui::PtrRoles<Node>{list->head, prev, curr, next, spot});
            std::cout << "C (isolated): [" << curr->data << "]\n\n";
#endif
            //
            ...
            if (spot == nullptr) {
...
#ifdef TRACE
                traceui::print_state("State: AFTER insert at head",
                                     list->head,
                                     traceui::PtrRoles<Node>{list->head, prev, curr, next, spot});
#endif
                //
                ...
                DebugPrint("Inserted curr", list);
#ifdef TRACE
                traceui::print_state("State: AFTER insert after S (spot)",
                                     list->head,
                                     traceui::PtrRoles<Node>{list->head, prev, curr, next, spot});
#endif
            }
...
void PushBack(List* list, int data) {
    ...
    if (!list->head) { list->head = n; return; }
    Node* curr = list->head;
